...
class ChromaDBSetup {
    logger = new common_1.Logger(ChromaDBSetup.name);
    client;
    ...
    async addSampleData(collection) {
...
- TypeScript and NestJS backend`,
        ...
        this.logger.log(`  - ${sampleData.metadatas.filter(m => m.type === chromadb_constant_1.CHROMA_DOCUMENT_TYPES.JOB_DESCRIPTION).length} job descriptions`);
        this.logger.log(`  - ${sampleData.metadatas.filter(m => m.type === chromadb_constant_1.CHROMA_DOCUMENT_TYPES.CASE_STUDY).length} case study briefs`);
    }
}
exports.ChromaDBSetup = ChromaDBSetup;
const setup = new ChromaDBSetup();
void setup
    .run()
    .then(() => process.exit(0))
    .catch(() => process.exit(1));
//# sourceMappingURL=setup-chromadb.script.js.map